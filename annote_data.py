...
import torch
sys.path.append(r"D:\my_project\PROJECT_DETECT_OBJECT\scripts")
...
class ImageAnnotator(BaseAnnotator):
    ...
    def annotate(self):
        ...
        masks = self.model.mask_generator.generate(img)  # Trả về dictionary.
    ...
    def annotate(self):
        ...
        while cap.isOpened():
            ...
            if frame_count % self.frame_step == 0:  # annotate mỗi N frame
                ...
                masks = self.model.video_predictor.generate(frame)
...
class AnnotationPipeline:
    def __init__(self, checkpoint_path, config_path, raw_dir, annotated_dir):
        self.model = SAM2Model(checkpoint_path, config_path, device='cpu')
...
if __name__ == "__main__":
    project_root = r"D:\my_project\PROJECT_DETECT_OBJECT"  # đường dẫn gốc của dự án
    pipeline = AnnotationPipeline(
        checkpoint_path=os.path.join(
            project_root, r"sam2_repo\checkpoints\sam2.1_hiera_large.pth"),
        config_path=os.path.join(
            project_root, r"sam2_repo\configs\sam2.1_hiera_l.yaml"),
        raw_dir=os.path.join(project_root, r"data\raw"),
        annotated_dir=os.path.join(project_root, r"data\annotated")
    )
    pipeline.run()
